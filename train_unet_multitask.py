...
from smistad.smistad_network import Unet, Unet_MTL
...
name = '171020_multiclass_nuclei_seg_256_32_rgb_nbclasses_2_bs_96_all_augs'
...
# define model
mtl_flag = True  # FIXME: quick-fix because I'm lazy
network = Unet_MTL(input_shape=input_shape, nb_classes=nb_classes)
...
if mtl_flag:
    model.compile(
        ...
        loss=[network.get_dice_loss(), "categorical_crossentropy"],
...
train_gen = batch_gen2(train_dir, batch_size=batch_size, aug=train_aug, epochs=epochs, input_shape=(256, 256, 3),
                       nb_classes=nb_classes, N_samples=N_train, mtl_flag=mtl_flag)
val_gen = batch_gen2(val_dir, batch_size=batch_size, aug=val_aug, epochs=epochs, input_shape=(256, 256, 3),
                     nb_classes=nb_classes, N_samples=N_val, mtl_flag=mtl_flag)
