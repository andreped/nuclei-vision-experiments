...
def batch_gen2(file_list, batch_size, aug={}, epochs=1, input_shape=(256, 256, 3), nb_classes=2, N_samples=100):
    for i in range(epochs):
        ...
        for n in range(N_samples):
            ...
            image_w = 256; image_h = 256

            label_matrix = np.zeros([7, 7, 6])  #6])  # <- [7, 7, 25] as the original dataset had 20 (?) classes
            #for l in label:
            for ii in range(label.shape[0]):
                l = label[ii].astype(np.int)
                # l = l.split(',')
                ...
                xmax = l[3]
                ymax = l[2]
                cls = 1 #l[4] # <- only 1 class of interest (nuclei all in one class)
                ...
                #'''
                if label_matrix[loc_i, loc_j, nb_classes+3] == 0:
                    label_matrix[loc_i, loc_j, cls] = 1
                    label_matrix[loc_i, loc_j, :(nb_classes+3)] = [x, y, w, h]
                    label_matrix[loc_i, loc_j, nb_classes+3] = 1  # response
                #'''

                '''
                if label_matrix[loc_i, loc_j, 24] == 0:
                    label_matrix[loc_i, loc_j, cls] = 1
                    label_matrix[loc_i, loc_j, :24] = [x, y, w, h]
                    label_matrix[loc_i, loc_j, 24] = 1  # response
                '''

            gt.append(label_matrix)
