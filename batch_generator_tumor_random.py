...
def add_shift2(input_im, output, max_shift):
    ...
    output[..., 0] = shift(output[..., 0], sequence[:-1], order=0, mode='constant', cval=1)
    output[..., 1] = shift(output[..., 1], sequence[:-1], order=0, mode='constant', cval=0)
...
def add_scaling2(input_im, output, r_limits):
    ...
    def crop_or_fill(image, shape, pad=0):
        image = np.copy(image)
        for dimension in range(2):
            if image.shape[dimension] > shape[dimension]:
                ...
                # Fill
                if dimension == 0:
                    new_image = np.zeros((shape[0], image.shape[1], shape[2])) + pad
                    new_image[:image.shape[0], :, :] = image
                elif dimension == 1:
                    new_image = np.zeros((shape[0], shape[1], shape[2])) + pad
    ...
    output[..., 0] = crop_or_fill(scipy.ndimage.zoom(np.expand_dims(output[..., 0], -1).astype(np.float32), [scaling_factor, scaling_factor, 1], order=0), output.shape + (1,), pad=1)[..., 0]
    output[..., 1:] = crop_or_fill(scipy.ndimage.zoom(output[..., 1:].astype(np.float32), [scaling_factor, scaling_factor, 1], order=0), output[..., 1:].shape)
    #output[..., 0] = 1 - output[..., 1]
...
def batch_gen2(file_list, batch_size, aug={}, epochs=1, input_shape=(256, 256, 3), nb_classes=2, N_samples=100, mtl_flag=False):
    for i in range(epochs):
        ...
        gt_classes = np.zeros((batch_size, nb_classes), dtype=np.float32)
        ...
        for n in range(N_samples):
            ...
            for ii in range(output.shape[-1]):
                ax[ii+1].imshow(output[..., ii], cmap="gray")
            plt.show()
            '''
            ...
            # calculate percentage of nuclei percent to produce classification labels (only works for two classes!)
            if mtl_flag:
                tmp = (np.mean(output[..., 1]) > 0.01).astype(int)
                gt_class_curr = np.zeros(2, dtype=np.float32)
                gt_class_curr[tmp] = 1
                gt_classes[batch] = gt_class_curr
                del gt_class_curr, tmp
